...
// PRO primary nav (4 items + More sheet)
const PRIMARY_NAV = [
...
// Admin sidebar — mirrors AdminPage TABS order
const ADMIN_NAV = [
...
export default function Layout({ children }) {
  ...
  function navToTab(id) { window.location.hash = "#" + id; setActiveTab(id); }

  // ── Admin — fixed left sidebar + scrollable content ──────────────────────
  if (admin) {
    return (
      <div className="admin-layout" style={{ display: "flex", minHeight: "100vh", background: "#F7F8FA" }}>
        ...
        <div style={{
          ...
          <div style={{ padding: "18px 18px 14px", borderBottom: `1px solid ${C.border}`,
            ...
            <VSAPin size={32} />
            <div>
              <div style={{ color: C.brand, fontWeight: 800, fontSize: 14,
                letterSpacing: 0.5, lineHeight: 1.2 }}>VSA FIELD</div>
              <div style={{ color: C.faint, fontSize: 10, letterSpacing: 0.3,
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "10px 10px 6px" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.faint, letterSpacing: 0.8,
              textTransform: "uppercase", padding: "6px 12px 8px" }}>Sections</div>
            {ADMIN_NAV.map(item => {
              const active = activeTab === item.id;
              return (
                <div key={item.id} onClick={() => navToTab(item.id)} style={{
                  display: "flex", alignItems: "center", gap: 10,
                  padding: "9px 12px", borderRadius: 9, cursor: "pointer", marginBottom: 1,
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, padding: "14px 16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 36, height: 36, background: C.brand, borderRadius: 10,
              ...
              <div style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${C.border}`,
          ...
          return (
            <div key={item.path} onClick={() => { setMoreOpen(false); nav(item.path); }}
              style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center",
                ...
                color: active ? C.brand : C.faint, transition: "color 0.15s", position: "relative" }}>
              {active && (
            ...
            {MORE_NAV.map(item => {
              ...
              return (
                <div key={item.path} onClick={() => { setMoreOpen(false); nav(item.path); }}
                  ...
                  {active && (
                    <Icon name="check-circle" size={16} color={C.brand} style={{ marginLeft: "auto" }} />
                  )}
                </div>
              );
            })}
