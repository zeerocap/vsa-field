...
export const ADMIN_TAB_IDS = [
  "overview","activities","live","trail","sessions","venues","leads",
  "targets","expenses","map","territory","photos","logins","faceid",
];
...
function ExpensesTab({ authUser, isMobile }) {
  ...
  const load = useCallback(async () => {
    ...
    if (r.ok) { setItems(r.expenses || []); setError(""); }
  ...
  return (
    <div>
      ...
      {error && (
        <div style={{ marginBottom: 14, padding: "10px 14px", background: C.dangerBg,
          border: `1px solid ${C.danger}40`, borderRadius: 8, fontSize: 13, color: C.danger, fontWeight: 600 }}>
          {error}
        </div>
      )}
            ...
            <thead>
              <tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                {["PRO", "Date", "Category", "Amount", "Notes", "Status", ""].map((h, i) => (
                  <th key={h} style={{ padding: "10px 14px", fontSize: 11, fontWeight: 700, color: C.muted,
              ...
              {rows.map(e => {
                ...
                return (
                  <tr key={e.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                    ...
                    <td style={{ padding: "10px 14px", color: C.muted, whiteSpace: "nowrap" }}>{e.date}</td>
                    <td style={{ padding: "10px 14px", color: C.text }}>{e.category}</td>
                    <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: C.text }}>{money(e.amount)}</td>
                    <td style={{ padding: "10px 14px", color: C.muted, maxWidth: 260 }}>{e.notes || "—"}</td>
                    <td style={{ padding: "10px 14px" }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 9px", borderRadius: 99,
                        background: `${chip[st]}18`, color: chip[st], textTransform: "capitalize" }}>{st}</span>
                    </td>
                    <td style={{ padding: "10px 14px", textAlign: "right", whiteSpace: "nowrap" }}>
                      {st === "pending" && (
                        <>
                          <button disabled={busy === e.id} onClick={() => review(e.id, "approved")}
                            style={{ padding: "5px 12px", fontSize: 12, fontWeight: 600, borderRadius: 7,
                              border: `1px solid ${C.success}`, background: C.success, color: "#fff",
                              cursor: "pointer", fontFamily: "inherit", marginRight: 6 }}>
                            {busy === e.id ? "…" : "Approve"}
                          </button>
                          <button disabled={busy === e.id} onClick={() => review(e.id, "rejected")}
                            style={{ padding: "5px 12px", fontSize: 12, fontWeight: 600, borderRadius: 7,
                              border: `1px solid ${C.danger}`, background: C.card, color: C.danger,
...
function FieldMarketingInner({ authUser, isMobile }) {
  ...
  return (
    <div style={{ maxWidth:"100%" }}>
      ...
      {tab === "expenses"  && <ExpensesTab  authUser={authUser} isMobile={isMobile}/>}
