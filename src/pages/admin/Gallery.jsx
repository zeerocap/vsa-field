    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {withPhoto.length === 0 ? <Empty msg="No check-in photos yet" icon="📸" /> : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 10 }}>
          {withPhoto.map((s, i) => (
            <div key={s.id || i} onClick={() => setPreview(s)}
              style={{ cursor: "pointer", borderRadius: 12, overflow: "hidden", background: C.card, border: `1px solid ${C.border}`, boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}>
              <img src={s.selfie_photo} alt="check-in"
                style={{ width: "100%", aspectRatio: "1", objectFit: "cover", display: "block" }} />
              <div style={{ padding: "8px 10px" }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.pro_username}</div>
                <div style={{ fontSize: 10, color: C.muted }}>{s.check_in_at ? new Date(s.check_in_at).toLocaleDateString() : ""}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {preview && (
        <div onClick={() => setPreview(null)}
          ...
          <div style={{ color: "#fff", marginTop: 12, fontSize: 14, fontWeight: 600 }}>{preview.pro_username}</div>
