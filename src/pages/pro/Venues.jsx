...
export default function ProVenues() {
  ...
  useEffect(() => {
    getVenues()
      .then((r) => {
        // call() resolves {ok:false} instead of throwing — check ok or a failed
        // load looks like "no venues".
        if (!r.ok) {
          setLoadErr(r.error || "Could not load venues.");
          return;
        }
        setVenues(r.venues || []);
      })
            ...
            <Card key={v.id} style={{ padding: 16, border: `1px solid ${C.border}` }}>
              ...
              >
                ...
                <div style={{ flex: 1, padding: "10px 12px", textAlign: "center" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.info }}>
                    {timeAgo(v.last_visited_at)}
                  </div>
                  <div style={{ fontSize: 10, color: C.muted, fontWeight: 600, marginTop: 2 }}>
                    LAST VISIT
                  </div>
              ...
              {(v.contact_name || v.contact_phone) && (
                ...
                >
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: C.brand }}>
                    {v.contact_name}
                  </span>
                  {v.contact_phone && (
                    <a
