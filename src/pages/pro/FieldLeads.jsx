...
export default function ProFieldLeads() {
  ...
  const load = () => {
    ...
    Promise.all([getFieldLeads({}), getVenues()])
      .then(([l, v]) => {
        // call() resolves {ok:false} instead of throwing, so the .catch never
        // fires — check ok here or a failed load looks like an empty leads list.
        if (!l.ok) {
          setLoadErr(l.error || "Could not load leads.");
          return;
        }
        setLeads(l.leads || []);
        setVenues(v.venues || []);
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      const res = await addActivity({
        venueId: form.venue_id || undefined, // UUID string — never Number()
      ...
      if (!res.ok) {
        setErr(res.error || "Failed to save lead");
        return;
      }
      setOpen(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {filtered.length === 0 ? (
        <Empty
          msg={search ? "No leads match your search" : "No field leads yet"}
          icon="users"
        ...
        filtered.map((l, i) => {
          const sc = STATUS_COLOR[l.crm_status] || STATUS_COLOR.new;
          return (
            <Card key={i} style={{ padding: "14px 16px" }}>
              ...
              >
                <div>
                  <div style={{ fontWeight: 700, fontSize: 15, color: C.text }}>{l.full_name}</div>
                  <a
                    href={`tel:${l.phone}`}
                    style={{
                      display: "flex",
        ...
        <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <Select
              label="Venue"
              value={form.venue_id}
              onChange={(e) => F("venue_id", e.target.value)}
              options={[
                { value: "", label: "— Venue —" },
