...
export default function ProExpenses() {
  ...
  const load = () => {
    ...
    getExpenses({})
      .then((r) => {
        // call() resolves {ok:false} instead of throwing — check ok here or a
        // failed load looks like "no expenses yet".
        if (!r.ok) {
          setLoadErr(r.error || "Could not load expenses.");
          return;
        }
        setItems(r.expenses || []);
      })
  ...
  // status is a string (pending/approved/rejected), not an `approved` boolean —
  // only genuinely pending claims count toward the amount awaiting approval.
  const pending = items
