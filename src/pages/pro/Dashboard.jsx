...
export default function ProDashboard() {
  ...
  useEffect(() => {
    Promise.all([getFieldDashboard(), getTodayStatus(), getTargets({})])
      .then(([d, s, t]) => {
        // call() resolves {ok:false} rather than throwing, so the .catch never
        // fires — without this an outage shows every KPI as 0, indistinguishable
        // from a PRO who genuinely did nothing today.
        if (!d.ok) {
          setLoadErr(d.error || "Could not load your dashboard.");
          return;
        }
        setData(d);
        setStatus(s);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* Targets preview */}
      {targets.some((t) => (t.target_visits || 0) > 0 || (t.target_leads || 0) > 0) && (
        <Card style={{ padding: 16 }}>
          ...
          >
            <div style={{ fontWeight: 700, fontSize: 14, color: C.text }}>Monthly Targets</div>
            <div
              onClick={() => nav("/targets")}
          ...
          {targets
            ...
            .slice(0, 1)
            .map((t) => (
              <div key={t.month_year} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  { label: "Visits", val: t.achieved_visits || 0, max: t.target_visits || 0 },
                  { label: "Leads", val: t.achieved_leads || 0, max: t.target_leads || 0 },
                ]
                  .filter((r) => r.max > 0)
                  .map((r) => (
                    <div key={r.label}>
                      ...
                      <ProgressBar value={r.val} max={r.max} showLabel={false} height={6} />
                    </div>
                  ))}
              </div>
            ))}
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                  ...
                  {a.leads_captured
                    ? ` · ${a.leads_captured} lead${a.leads_captured !== 1 ? "s" : ""}`
                    : ""}
                </div>
              </div>
              <div
