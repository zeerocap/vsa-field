...
const TYPES = [
  "Venue Visit", "Demo", "Meeting", "Event", "Door-to-Door",
  "School Visit", "College Visit", "Corporate Visit", "Other",
];
...
export default function ProActivities() {
  ...
  const [form,    setForm]    = useState({
    venue_id: "", activity_type: "Venue Visit", leads_captured: "",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {items.length === 0 ? (
        <Empty msg="No activities yet" icon="clipboard" />
      ) : items.map((a, i) => (
        <Card key={a.id || i} style={{ padding: "14px 16px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                {fmtDate(a.activity_date)}{a.venue_name ? ` · ${a.venue_name}` : ""}
              </div>
            </div>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
            {a.duration_min && (
              <span style={{ background: C.infoBg, color: C.info, borderRadius: 20, padding: "2px 8px", fontSize: 11 }}>
                {a.duration_min} min
              </span>
            )}
            {a.is_late_entry && (
              <span style={{ background: C.warningBg, color: C.warning, borderRadius: 20, padding: "2px 8px", fontSize: 11, fontWeight: 600 }}>
                Late Entry
              </span>
            )}
            {(a.fake_gps_flag || a.gps_flag) && (
              <span style={{ background: C.dangerBg, color: C.danger, borderRadius: 20, padding: "2px 8px", fontSize: 11, fontWeight: 600 }}>
                GPS Flag
              </span>
            )}
          </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Log Activity">
        <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: C.text, display: "block", marginBottom: 6 }}>Date</label>
            <input
              type="date"
          ...
          <Select label="Activity Type" value={form.activity_type} onChange={e => F("activity_type", e.target.value)}
            required options={TYPES.map(t => ({ value: t, label: t }))} />
          <Input label="Leads Captured" value={form.leads_captured} onChange={e => F("leads_captured", e.target.value)}
            type="number" placeholder="How many leads?" />
          <Input label="Duration (minutes)" value={form.duration_min} onChange={e => F("duration_min", e.target.value)}
