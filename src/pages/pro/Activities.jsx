...
export default function ProActivities() {
  ...
  const load = () => {
    ...
    Promise.all([getActivities({}), getVenues()])
      .then(([a, v]) => {
        // call() resolves {ok:false} rather than throwing — check ok here or a
        // failed load renders as an empty activity list instead of an error.
        if (!a.ok) {
          setLoadErr(a.error || "Could not load activities.");
          return;
        }
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Log Activity">
        <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Select
            ...
            onChange={(e) => F("activity_type", e.target.value)}
            required
          ...
          <Select
            ...
            options={[
              { value: "", label: "— Select venue (optional) —" },
              ...venues.map((v) => ({ value: String(v.id), label: v.name })),
            ]}
          />
          <Input
            label="Leads Captured"
            value={form.leads_captured}
            onChange={(e) => F("leads_captured", e.target.value)}
            type="number"
            min="0"
            placeholder="0"
          />
