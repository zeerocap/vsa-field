...
export default function ProFieldLeads() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {leads.length === 0 ? <Empty msg="No field leads yet" icon="👥" /> : leads.map((l, i) => (
        <Card key={i} style={{ padding: "14px 16px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ background: C.bg, color: C.muted, borderRadius: 20, padding: "2px 8px", fontSize: 11 }}>{l.source}</span>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Add Field Lead">
        <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Select label="Course Interest" value={form.course} onChange={e => F("course", e.target.value)} options={COURSES.map(c => ({ value: c, label: c }))} />
          <Select label="Source" value={form.source} onChange={e => F("source", e.target.value)} options={SOURCES.map(s => ({ value: s, label: s }))} />
          ...
          <Btn type="submit" disabled={saving} size="lg" style={{ width: "100%" }}>{saving ? "Saving..." : "Add Lead"}</Btn>
        </form>
      </Modal>
    </div>
  );
